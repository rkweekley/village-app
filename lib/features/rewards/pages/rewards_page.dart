            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  controller: nameCtrl,
                  validator: (v) =>
                      (v?.trim() ?? '').isEmpty ? 'Enter a reward name' : null,
                  decoration: InputDecoration(
                    labelText: 'Reward name',
                    prefixIcon: const Icon(Icons.emoji_events_outlined),
                    filled: true,
      ...
      data: (rewards) {
        ...
        if (visible.isEmpty) {
          return EmptyState(
            ...
            subtitle: isParent
                ? 'Tap + to create one'
                : 'Ask a parent to create rewards',
...
class _RedemptionsTab extends StatelessWidget {
  ...
  final WidgetRef ref;
  const _RedemptionsTab({required this.redemptionsAsync, required this.ref});
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isPending
                              ...
                              : isApproved
                              ? context.palette.positive.withValues(alpha: 0.12)
                              : context.palette.danger.withValues(alpha: 0.12),
                          borderRadius: BorderRadius.circular(14),
                        ),
                        child: Icon(
                          isPending
                          ...
                          onPressed: () async {
                            try {
                              ...
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Redemption rejected'),
                                  ),
                                );
                              }
                            } catch (e) {
